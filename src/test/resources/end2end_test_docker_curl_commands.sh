...
docker run -p 8080:8080 -p 8001:8001 -d --name drools-workbench --env KIE_ADMIN_USER=kieserver --env KIE_ADMIN_PWD=kieserver1! jboss/drools-workbench-showcase:7.18.0.Final

# Docker KIE container
docker run -p 8180:8080 -d --name kie-server --link drools-workbench:kie-wb \
	...
	--env BC_MAVEN_REPO_URL=http://localhost:8080/business-central/maven2 \
...
curl http://localhost:8080/business-central/rest/spaces -X POST --data '{"name": "MySpace","description": "My new space.","owner": "admin","defaultGroupId": "com.newspace"}' \
...
curl http://localhost:8080/business-central/rest/spaces/MySpace/git/clone -X POST --data '{"name":"xavier-analytics","description":"project description inside business central.","gitURL":"https://github.com/jonathanvila/xavier-analytics"}' -H "Cache-Control:no-cache" -H "Content-Type:application/json" -H "Authorization:Basic YWRtaW46YWRtaW4=" -v

# Compile the project
curl http://localhost:8080/business-central/rest/spaces/MySpace/projects/Xavier%20Analytics/maven/compile -X POST -H "Cache-Control:no-cache" -H "Content-Type:application/json" -H "Authorization:Basic YWRtaW46YWRtaW4=" -v

# Install the project
curl http://localhost:8080/business-central/rest/spaces/MySpace/projects/Xavier%20Analytics/maven/install -X POST -H "Cache-Control:no-cache" -H "Content-Type:application/json" -H "Authorization:Basic YWRtaW46YWRtaW4=" -v

# Deploy the project
curl http://localhost:8080/business-central/rest/spaces/MySpace/projects/Xavier%20Analytics/maven/deploy -X POST -H "Cache-Control:no-cache" -H "Content-Type:application/json" -H "Authorization:Basic YWRtaW46YWRtaW4=" -v
...
curl  -X PUT -H "Cache-Control:no-cache" -H "Content-Type:application/json" -H "Authorization:Basic a2llc2VydmVyOmtpZXNlcnZlcjEh" --data '{"container-id" : "xavier-analytics_0.0.1-SNAPSHOT","release-id" : {"group-id" : "org.jboss.xavier","artifact-id" : "xavier-analytics","version" : "0.0.1-SNAPSHOT" } }' http://localhost:8180/kie-server/services/rest/server/containers/xavier-analytics_0.0.1-SNAPSHOT -v
